...
document.addEventListener('DOMContentLoaded', async () => {
    await loadUserData();
    await loadHistory();
    document.getElementById('profilePic').textContent = user.email[0].toUpperCase();
...
function showSymptomResult(data) {
    ...
    const severityClass = data.diagnosis.severity === 'High' ? 'btn-danger' : '';
        ...
        hospitalsHtml = `
            <div class="hospital-list">
                ...
                ${data.hospitals.map(h => `
                    <div class="hospital">
                        <span><strong>${h.name}</strong> (${h.distance})</span>
                        <a href="tel:${h.contact}" style="color:#4f46e5;">${h.contact}</a>
                    </div>
                `).join('')}
            </div>
        `;
    ...
    resultDiv.innerHTML = `
        <div class="result">
            <h4><i class="fas fa-diagnoses"></i> ${data.diagnosis.disease}</h4>
            <p><strong>Severity:</strong> ${data.diagnosis.severity}</p>
            <p><strong>Precautions:</strong> ${data.diagnosis.precautions}</p>
            <p><strong>Medicine:</strong> ${data.diagnosis.medicine}</p>
            ${data.diagnosis.emergency ? '<p style="color:#ef4444;"><strong>🚨 EMERGENCY: Seek immediate medical help!</strong></p>' : ''}
            ${hospitalsHtml}
            <button class="btn ${severityClass}" onclick="bookDoctor()" style="margin-top:15px;">Book Doctor</button>
        </div>
    `;
}
...
document.getElementById('fileInput').addEventListener('change', async (e) => {
    ...
    try {
        const res = await fetch(`${API_BASE}/upload/lab-report`, {
            method: 'POST',
        ...
        document.getElementById('labResult').innerHTML = `
            <div class="result">
                <h4><i class="fas fa-file-alt"></i> Lab Analysis</h4>
                <p>${data.text}</p>
                <details>
...
async function updateProfile() {
    ...
    // Save to localStorage for now (add API later)
        ...
        if (history.length) {
            historyList.innerHTML = history.map(h => `
                <div style="padding:10px; border-bottom:1px solid #eee;">
                    <strong>${h.diagnosis}</strong> - ${new Date(h.date).toLocaleDateString()}
                    <br><small>${h.symptoms}</small>
                </div>
            `).join('');
        }
    } catch (error) {
...
function bookDoctor() {
    alert('Doctor booking feature coming soon! Call nearby hospital.');
}
